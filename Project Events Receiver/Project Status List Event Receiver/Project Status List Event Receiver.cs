...
namespace Project_Events_Receiver.Project_Status_List_Event_Receiver
{
    /// <summary>
    /// 列表项事件
    /// </summary>
    public class Project_Status_List_Event_Receiver : SPItemEventReceiver
    {
       /// <summary>
       /// 已添加项.
       /// </summary>
	   ...
	   protected void assign_role(SPSite site, SPListItem item, SPListItem project, string field_name, string field_guid, SPRoleDefinition definition)
	   {
		   ...
		   {
			   log(site, "为项目【" + project["Title"] + "】的【" + field_name + "】的状态授权时发生错误", "错误", ex.ToString());
		   }
	   }
	   ...
	   protected void assign_roles(SPWeb web, SPListItem item, SPListItem project, string field_name, string field_guid, SPRoleDefinition definition)
	   {
		   ...
		   {
			   log(web.Site, "为项目【" + project["Title"] + "】的【" + field_name + "】的状态授权时发生错误", "错误", ex.ToString());
		   }
	   }
